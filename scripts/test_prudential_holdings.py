...
def is_holdings_end(
    ...
    # Some Prudential factsheets glue a footnote marker directly onto
    # the start of an end-of-section line, e.g. "3Source: Schroder..."
    # or "4Important Information". Strip a leading 1-2 digit footnote
    # marker ONLY when it is directly touching a following letter (no
    # space), so this never affects percentages ("1.2%"), years
    # ("2026"), or anything else that happens to start with a digit.
    normalized = re.sub(
        r"^[0-9]{1,2}(?=[a-z])",
        "",
        normalized,
